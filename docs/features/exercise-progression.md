...
- Elastic-band exercises (e.g., `equipment: "elastic-band"`) are always available — there is no equipment gate. Their status in `getExerciseStatuses` and inclusion in `getAvailableExercises` depend only on unlock/retirement, exactly like every other exercise. The per-exercise `equipment` tag still drives cosmetic touches (the in-workout band note and the exercise-library badge).
