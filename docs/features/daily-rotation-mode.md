...
- **Upper body not yet wired**: The rotation currently cycles only abs → glutes → lowerBack. A fourth `upperBody` group and its slot-tagged exercises already exist in the library (see [upper-body-exercises.md](upper-body-exercises.md)) as the data foundation, but adding it to the rotation sequence, generator, and stretching map is a follow-on.
